...
export default class Store {
    ...
    playerMove(squareId) {
        const stateClon = structuredClone(this.#getState());

        stateClon.currentGameMoves.push({
            squareId,
            player: this.game.currentPlayer
        })

        this.#saveState(stateClon);
    }
    ...
    reset() {
        ...
        if (status.isComplete) {
            stateClon.history.currentRoundGames.push({
                moves,
                status,
            });
        }

        stateClon.currentGameMoves = [];
